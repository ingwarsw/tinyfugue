    ...
    for ( ; *s; s++) {
	    ...
	    attr_t new = attrs;
            ...
            do {
                int rgbctlseq = 0;
                s++;
                if (strncmp(s, "38;2;", 5) == 0 || strncmp(s, "38:2:", 5) == 0)
                    rgbctlseq = 1;
                else if (strncmp(s, "48;2;", 5) == 0 ||
                        strncmp(s, "48:2:", 5) == 0)
                    rgbctlseq = 2;
                else
                    i = strtoint(s, &s);
                if (rgbctlseq) {
                    /*
                     * we accept either colon- or semicolon-separated values,
                     * and either 3 or 4 values, for historical
                     * terminal-related reasons. see:
                     * https://invisible-island.net/xterm/ctlseqs/ctlseqs.html
                     * https://invisible-island.net/xterm/xterm.faq.html#color_by_number
                     * and the xterm source code.
                     */
                    unsigned space, r, g, b;
                    int matched;
                    s += 5; /* skip "38;2;" */
                    matched = sscanf(s, "%u;%u;%um", &r, &g, &b);
                    if (matched != 3)
                        matched = sscanf(s, "%u:%u:%um", &r, &g, &b);
                    if (matched != 3) {
                        matched = sscanf(s, "%u;%u;%u;%um", &space, &r, &g, &b);
                        if (matched != 4)
                            matched = sscanf(s, "%u:%u:%u:%um", &space, &r, &g, &b);
                        if (matched != 4)
                            continue;
